...
function checkStat() {
    if (!statedrop.value) {
        console.log("value");
        var formParent = statedrop.parentElement;
        var small = formParent.querySelector("small");
        small.className = "formerror";
        small.innerText = "state is invalid";
    } else {
        var formParent = statedrop.parentElement;
        var small = formParent.querySelector("small");
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  //to put values of city based on state
...
function checkDateofbirth() {
  ...
  //  var match = dob.value.match(/^\d{4}/);
    
    if (!dob.value) {
        console.log("gotin");
        setErrorFor(dob, "date of birth required");
        return false;
    } else if (!isOld(dob.value)) {
